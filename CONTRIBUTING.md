...
3.  Make your changes. Follow the [tidyverse style
    guide](https://style.tidyverse.org/).

4.  Document new functions with **roxygen2** tags and run
    `devtools::document()`.

5.  Add or update **testthat** tests. Aim for \> 90 % line coverage.

6.  Run the full check suite and confirm it passes with no warnings or
    notes:
...
7.  Submit a pull request with a clear description of what changed and
    why.
...
- Use `\(x)` lambda syntax (R ≥ 4.1) rather than `function(x)` for short
  anonymous functions.
- All user-facing functions must have complete roxygen2 documentation
  including at least one `@examples` block.
- Internal helpers (prefixed with `.`) do not require exported
  documentation but should have inline comments.
